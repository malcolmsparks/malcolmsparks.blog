(ns malcolmsparks.blog.generate
  (:require
...
(defn -main [& args]
  (eval '(do (require 'malcolmsparks.blog.system)
             (require 'malcolmsparks.blog.generate)
             ...
             (let [system (->
               ...
               (binding [malcolmsparks.blog.generate/*handler* (-> system :modular-bidi-router-webrouter modular.ring/request-handler)]
                 (malcolmsparks.blog.generate/do-actions
                  (mapcat (partial malcolmsparks.blog.generate/spider #{}) [{:uri "/"}])))

               (println "Stopping malcolmsparks.blog")
